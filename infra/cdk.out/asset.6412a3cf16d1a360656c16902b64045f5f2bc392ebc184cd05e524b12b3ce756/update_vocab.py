...
def lambda_handler(event, context):
    ...
    try:
        ...
        if action == 'update_book':
            ...
            return update_question_scores(cursor, data)
        elif action == 'test_update':
            return test_database_update(cursor)
        else:
            return lambda_response(400, {
                'error': 'Invalid action',
                'message': 'Action must be update_book, update_question, update_scores, or test_update'
            })
...
def update_question_scores(cursor, data):
    """語彙質問のスコアを更新（学習進捗管理用）"""
    question_id = data.get('id')
    ...
    for score_field in ['s0', 's2', 's3', 's4', 's5', 's7']:
        if score_field in scores:
            score_fields.append(f"{score_field} = %s")
            score_values.append(int(scores[score_field]))
    
    if not score_fields:
        return lambda_response(400, {
            'error': 'No scores to update',
            'message': 'At least one score field (s0, s2, s3, s4, s5, s7) must be provided'
        })
    
    score_values.append(question_id)
    
    cursor.execute(f"""
        UPDATE vocabulary_questions 
        SET {', '.join(score_fields)}, updated_at = CURRENT_TIMESTAMP
        WHERE id = %s
        RETURNING id, ka, np1, jp_kanji, jp_rubi, s0, s2, s3, s4, s5, s7, updated_at
    """, score_values)
    ...
    return lambda_response(200, {
        'message': 'Question scores updated successfully',
        'question': question
    })
...
def test_database_update(cursor):
    ...
    # そのブックの最初の質問のスコアを更新
    cursor.execute("""
        SELECT id FROM vocabulary_questions 
        WHERE book_id = %s ORDER BY renban LIMIT 1
    """, (book_id,))
    ...
    if question_row:
        ...
        cursor.execute("""
            UPDATE vocabulary_questions 
            SET s0 = s0 + 1, s2 = s2 + 1, updated_at = CURRENT_TIMESTAMP
            WHERE id = %s
            RETURNING s0, s2, updated_at
        """, (question_id,))
        ...
        return lambda_response(200, {
            ...
            'updated_question': {
                'id': question_id,
                's0': updated_question[0],
                's2': updated_question[1],
                'updated_at': updated_question[2]
            }
        })
    else:
        return lambda_response(200, {
            'message': 'Book updated successfully (no questions found)',
            'updated_book': {
                'id': book_id,
                'name': updated_book[0],
                'updated_at': updated_book[1]
